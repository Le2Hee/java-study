...
//																	-> 구현하는 클래스는 따로.
// DAO(Date Access Object) -> 데이터를 접근하는 오브젝트
//  -> 데이터를 사용해서 비즈니스 로직(기능)을 수행하는 객체.
...
public interface ContactDao {
	...
	/**
	 * 인덱스로 연락처 검색 기능. 전달받은 인덱스 위치에 있는 연락처 객체를 리턴하기
	 * @param index 검색할 인덱스. 0 이상의 정수. 
	 * @return 해당 인덱스 연락처(Contact) 객체 또는 null.
	 */
	Contact read(int index);
	
	/**
	 * 인덱스 수정 기능. 전달받은 인덱스 위치에 있는 연락처 객체를 수정하기.
	 * @param index 수정할 연락처의 인덱스. 0 이상이고, 저장된 연락처 개수보다 작은 정수.
	 * @param contact 수정할 연락처 정보를 가진 객체.
	 * @return 수정하면 1, 실패하면 0을 리턴.
	 */
	int update(int index, Contact contact);
	
	/**
	 * 인덱스 삭제 기능. 전달받은 인덱스 위치에 있는 연락처 객체를 삭제하기.
	 * 
	 * @param index 삭제할 연락처의 인덱스. 0이상이고, 저장된 연락처 개수보다 작은 정수.
	 * @return 삭제하면 1, 실패하면 0 리턴. 
	 */
