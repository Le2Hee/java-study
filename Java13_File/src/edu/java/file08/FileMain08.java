 ...
 *    Reader
 *    |__ InputStreamReader, BufferReader
 *       |__ FileReader
 *    
 *    Writer
 ...
 *     - MacOS, Linux : UTF-8
 *     
 *  Java 11 이전 버전까지는 FileReader와 FileWriter에서 인코딩 설정을 할 수 없었다.
 *  IntputStreamReader, OutputStreaWriter가 인코딩 설정 기능을 가지고 있었다.
 *  
 *  Java 11 이후 버전부터는 FileReader와 FileWriter에서도 인코딩 설정을 할 수 있께 업데이트됬다.
 *  
 *  txt 파일에 인코딩 소스를 맞게 선택해서 출력을 하면 깨짐현상이 없이 텍스트가 나온다.
 *   
 */
...
public class FileMain08 {
	...
	public static void main(String[] args) {
		...
		// FileReade, FileWriter에서 인코딩 방식을 설정할 수 있게됨.
		try (
				// 1. FileReader를 생성할 때 인코딩 방식을 설정.
				FileReader reade = new FileReader(utf8File, Charset.forName("UTF8"));
...
//				System.out.print((char) read);
				...
				// 한 줄씩 읽는 방법 -> 문자열
				String line = br.readLine(); // 1줄씩 읽는다.
				if (line == null) { // EOF
					break;
				}
