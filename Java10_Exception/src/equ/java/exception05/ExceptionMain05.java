 ...
 *    - 예외 처리를 하지 않아도(try-catch 문장을 사용하지 않아도)
 *      컴파일 에러가 발생하지 않는 예외 class들 
 *    - RuntimeException의 하위 클래스들.
 *    - (예) NumerFormatException, NullPointException, ...
 *    
 * 2. checked exception 
 *    - 반드시 예외 처리를 해야만 하는 예외 클래스들.
 *    - try-catch 문장을 사용하지 않거나 또는 
 *      throws 선언문을 사용하지 않으면 컴파일 에러가 발생하는 예외 클라스들.
