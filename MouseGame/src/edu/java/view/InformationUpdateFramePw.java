...
public class InformationUpdateFramePw extends JFrame {
	...
	private static InformationUpdateFramePw frame;

	/**
	 * Launch the application.
	 */
	public static void showInformationUpdateFramePw(Component parent, User user) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					frame = new InformationUpdateFramePw(parent, user);
	...
	public InformationUpdateFramePw(Component parent, User user) {
		this.user = user;
		this.parent = parent;
		initialize();
	}
	...
	private void initialize() {
		...
		btnNewButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				int result = dao.UpdatePW(textField.getText(), user);
				if (result == 0) {
